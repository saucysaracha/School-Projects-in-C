...
void executeAction(char c)
{
	...
	case 'c':		// change owner
		...
		else
		{
			...
			changeOwner(returnedVehicle, owner_input); //what is the vehicle we are using? returned vehicle? is that what current vehicle is called?
...
void addVehicle(string id_input, string owner_input, vehicleType type)
{
	...
	//Vehicle* newVehicle = new Vehicle(id_input, owner_input, type);
	//newVehicle = new Vehicle(id_input, owner_input, type);

	if (type == 0)
	{

		Car* newVehicle = new Car(id_input, owner_input, type);
		//newVehcle = new Car(id_input, owner_input, type);;
		newNode->vehicle = newVehicle;	//make new Vehicle and put it in new node
	}
	else
	{
		Motorcycle* newVehicle = new Motorcycle(id_input, owner_input, type);
		newNode->vehicle = newVehicle;	//make new Vehicle and put it in new node
	}
...
void displayList()
{
	...
	while (tempList != NULL) {
		tempList->vehicle->displayVehicle();
...
//	int save_file(string myFileName) { // myFileName is “contactFile”
		int count = 0;
		Container* temp = list;  // head is global
		...
		if (myFileVar.is_open()) {
			...
			while (temp != NULL) {
				//myFileVar << temp->vehicle << endl;
				myFileVar << temp->vehicle->getVehicleType() << endl;
				myFileVar << temp->vehicle->getOwner() << endl;
				myFileVar << temp->vehicle->getId() << endl;
				//now when you load, first thing you do is get vehicle type, based on that say new motorcycle or new car
				//do we copy every element of vehicle or just temp->vehicle?
				//like below?
				/*
				myFileVar << temp->vehicle->getId() << endl;
				myFileVar << temp->vehicle->getOwner() << endl;
...
void load(string fileName)
{
	...
	//void load_file(string myFileName) { // myFileName is “contactFile”
		...
		myFileVar.open(fileName); // open the same file name
		if (myFileVar.is_open()) {
			...
			for (int i = 0; i < count; i++) {
				...
				int type_input = 2;	//dummy value
				string id_input;
				string owner_input;

				//Container* newNode = new Container();
				
				//read vehicle type into temp
				vehicleType type;
				myFileVar >> type_input;
				myFileVar >> owner_input;
				myFileVar >> id_input;
				...
				if (type_input == 0)
				{
					...
					Car* newVehicle = new Car(id_input, owner_input, type);
					temp->vehicle = newVehicle;	//make new Vehicle and put it in new node
				}
				else
				{
					Motorcycle* newVehicle = new Motorcycle(id_input, owner_input, type);
